...
from .fdsn.fdsn_manager import (
    ...
    FdsnShakemapManager,
    FdsnWaveformManager,
    FdsnDataselectManager,
)

from .models import SearchEvent, SearchPeakMotions, SearchCombined, SearchCustom
from .forms import (
    SearchEventsForm,
    SearchPeakMotionsForm,
    SearchCombinedForm,
    SearchCustomForm,
)
    ...
    def get_sensor_channels_charts(self, motion_data):
        try:
            ...
            chart_pga = {
                ...
                "xAxis": {
                    "title": {"text": "Epicentral distance [km]"},
                    "type": "logarithmic",
                },
                "yAxis": {"title": {"text": "PGA [cm/s^2]"}, "type": "logarithmic"},
                "series": [],
            }
            ...
            chart_pgv = {
                ...
                "yAxis": {"title": {"text": "PGV [cm/s]"}, "type": "logarithmic"},
                "series": [],
            }
            ...
            for c in channels_pga:
                chart_pga["series"].append(
                    {"name": c, "data": [x for x in data_pga if x["channel"] == c]}
                )

            for c in channels_pgv:
                chart_pgv["series"].append(
                    {"name": c, "data": [x for x in data_pgv if x["channel"] == c]}
                )
    ...
    def get_spectra_charts(self, station_data):
        try:
            ...
            chart_psa = {
                ...
                "xAxis": {
                    "title": {"text": "Spectral period [s]"},
                    "type": "logarithmic",
                },
                "yAxis": {"title": {"text": "PSA [cm/s^2]"}, "type": "logarithmic"},
                "series": [],
            }
            ...
            chart_sd = {
                ...
                "yAxis": {"title": {"text": "SD [cm]"}, "type": "logarithmic"},
                "series": [],
            }

            for t in chart_data_psa:
                chart_psa["series"].append({"name": t, "data": chart_data_psa[t]})

            for t in chart_data_sd:
                chart_sd["series"].append({"name": t, "data": chart_data_sd[t]})
...
def search_events(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "search.html", {"form": form, "show_info": True})
    else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            data, ws_url = FdsnMotionManager().get_stations_list(
                pga_min=pga_min, pga_max=pga_max, pgv_min=pgv_min, pgv_max=pgv_max
            )
...
def search_custom(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(request, "search_custom.html", {"form": form})
    else:
